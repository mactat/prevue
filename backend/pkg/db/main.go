...
func SessionData(db *sql.DB, data types.SessionData) (int, error) {
	_, err := db.Exec(`INSERT INTO users (user_name, email, password)
	...
	modelId := 0
	err = db.QueryRow(`INSERT INTO Models (model_name, connector_name, architecture)
	VALUES ($1, $2, $3) RETURNING model_id;`,
		data.Models.ModelName,
		data.Models.ConnectorName,
		data.Models.Architecture).Scan(&modelId)
	if err != nil {
	...
	_, err = db.Exec(`INSERT INTO Projects (project_name, user_id, model_id)
	SELECT $1,
		   user_id,
		   $3
	FROM users
...
func MetricsData(db *sql.DB, data types.SessionMetrics) error {
	_, err := db.Exec(`INSERT INTO Metrics (model_id, epoch, batch, loss_name, loss_value, metric_name, metric_value, timestamp)
	VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
		data.Metrics.ModelId,
		data.Metrics.Epoch,
		data.Metrics.Batch,
		data.Metrics.LossName,
		data.Metrics.LossValue,
		data.Metrics.MatricsName,
		data.Metrics.MatricsValue, time.Now().UTC())
	if err != nil {
